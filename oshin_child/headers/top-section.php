<?php
/* mods 
 * 17Oct16 zig - for blog posts, setup a default hero section
 */
	$post_id = be_get_page_id();
	global $be_themes_data;
	//Identify the Source of Hero Section - 
	if((is_singular( 'post' ) /*|| is_singular('portfolio') */) && is_single($post_id) && isset($be_themes_data['single_blog_hero_section_from']) && $be_themes_data['single_blog_hero_section_from'] == 'inherit_option_panel') {
	...
	if ('op' == $hero_source) {
		...
		if($hero_section && !empty($hero_section) && $hero_section != 'none') {
			...
			echo '</div>';
		 	...
		 	echo '<div class="header-hero-section default-hero-section" id="hero-section">';
		 	echo '</div>';
		 }
	} 
	else {
